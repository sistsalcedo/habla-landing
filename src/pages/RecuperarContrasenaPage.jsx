  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { error } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: `${window.location.origin}/resetear`,
      })
        ...
        {success ? (
          <div className="rounded-lg border border-accent/50 bg-accent/10 px-4 py-4 text-accent">
            ...
            <p className="mt-1 text-sm text-text-muted">
              Hemos enviado un enlace a <strong className="text-white">{email}</strong>. Haz clic en él para crear una nueva contraseña.
            </p>
